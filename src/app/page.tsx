...
import { Activity, ArrowRight, CheckCircle2, Menu, X, BarChart3, Users, MessageSquare, CreditCard, TrendingUp, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  const features = [
    ...
    },
    {
      ...
      description: "No flat monthly fees to start. Buy prepaid credits and only consume them when a customer actually pays you.",
      icon: <CreditCard className="h-6 w-6 text-indigo-600" />
    },
    {
      title: "No Growth Penalty",
      description: "Scaling past a few hundred members? Switch to Unlimited and pay one flat price — forever. We're built to grow with your gym, not tax your success.",
  ...
  return (
    <main className="min-h-screen flex flex-col bg-slate-50 font-sans selection:bg-blue-100 selection:text-blue-900">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-2 font-bold text-xl tracking-tight text-slate-900">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] text-slate-500 tracking-wide mt-0.5 uppercase">PAY PER ACTIVE · NO GROWTH PENALTY</span>
              </div>
            </div>
      ...
      <section className="relative pt-20 pb-24 lg:pt-32 lg:pb-40 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="mt-6 text-lg md:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
            Built exclusively for small-scale gym owners. Start pay-as-you-go from ₹299, or go flat-rate from ₹599/month once you're established.
          </p>
      ...
      <section id="pricing" className="py-24 bg-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">ActiPay Fitness Pricing</h2>
            <p className="text-slate-400 text-lg">Pay-as-you-go to start. Simple flat pricing once you're established.</p>
          </div>
          
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto items-stretch">
            {/* Card 1 — Starter */}
            <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700 flex flex-col relative">
              <div className="inline-block self-start bg-slate-700 text-slate-200 text-[10px] font-black uppercase tracking-wider py-1 px-3 rounded-full mb-3">
                Pay As You Go
              </div>
              <h3 className="text-2xl font-bold mb-1">Starter</h3>
              <p className="text-slate-400 text-sm mb-6">30 member-months, ₹9.96 each</p>
              <div className="mb-6">
                <span className="text-4xl font-extrabold">₹299</span>
              </div>
              <ul className="space-y-3 mb-8 flex-1">
                <li className="flex items-center gap-3 text-sm text-slate-300">
                  <CheckCircle2 className="h-5 w-5 text-blue-400 shrink-0" /> Never expires
                </li>
                <li className="flex items-center gap-3 text-sm text-slate-300">
                  <CheckCircle2 className="h-5 w-5 text-blue-400 shrink-0" /> No subscription
                </li>
                <li className="flex items-center gap-3 text-sm text-slate-300">
                  <CheckCircle2 className="h-5 w-5 text-blue-400 shrink-0" /> Perfect for getting started
                </li>
              </ul>
              <Link href="/register" className="block text-center w-full py-3.5 px-4 rounded-full font-bold text-white bg-slate-700 hover:bg-slate-600 transition shadow-sm">
                Get Started
              </Link>
            </div>

            {/* Card 2 — Growth (Most Popular) */}
            <div className="bg-gradient-to-b from-blue-600 to-indigo-700 rounded-3xl p-8 border border-blue-400 flex flex-col relative transform md:-translate-y-4 shadow-2xl">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-amber-400 text-amber-950 text-[10px] font-black uppercase tracking-widest py-1 px-3.5 rounded-full shadow-md">
                Most Popular
              </div>
              <h3 className="text-2xl font-bold mb-1 text-white">Growth</h3>
              <p className="text-blue-200 text-sm mb-6">Up to 100 active members</p>
              <div className="mb-6">
              ...
              <ul className="space-y-3 mb-8 flex-1">
                <li className="flex items-center gap-3 text-sm text-white">
                  <CheckCircle2 className="h-5 w-5 text-emerald-400 shrink-0" /> Flat monthly price
                </li>
                <li className="flex items-center gap-3 text-sm text-white">
                  <CheckCircle2 className="h-5 w-5 text-emerald-400 shrink-0" /> No per-member math
                </li>
                <li className="flex items-center gap-3 text-sm text-white">
                  <CheckCircle2 className="h-5 w-5 text-emerald-400 shrink-0" /> Cancel anytime
                </li>
              </ul>
              <Link href="/register" className="block text-center w-full py-3.5 px-4 rounded-full font-bold text-blue-700 bg-white hover:bg-blue-50 transition shadow-lg">
                Start Free Trial
              </Link>
            </div>
            ...
            {/* Card 3 — Unlimited */}
            <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700 flex flex-col relative">
              <div className="inline-block self-start bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-[10px] font-black uppercase tracking-wider py-1 px-3 rounded-full mb-3">
                For Growing Gyms
              </div>
              <h3 className="text-2xl font-bold mb-1">Unlimited</h3>
              <p className="text-slate-400 text-sm mb-6">Unlimited active members</p>
              <div className="mb-6">
              ...
              <ul className="space-y-3 mb-8 flex-1">
                <li className="flex items-center gap-3 text-sm text-slate-300">
                  <CheckCircle2 className="h-5 w-5 text-indigo-400 shrink-0" /> One flat price, however big you grow
                </li>
                <li className="flex items-center gap-3 text-sm text-slate-300">
                  <CheckCircle2 className="h-5 w-5 text-indigo-400 shrink-0" /> No caps, ever
                </li>
              </ul>
              <Link href="/register" className="block text-center w-full py-3.5 px-4 rounded-full font-bold text-white bg-indigo-600 hover:bg-indigo-500 transition shadow-sm">
                Go Unlimited
              </Link>
            </div>
          </div>
          ...
          {/* Reassurance Callout */}
          <div className="mt-14 max-w-3xl mx-auto p-5 rounded-2xl bg-slate-800/80 border border-slate-700/80 text-center">
            <p className="text-sm text-slate-300 leading-relaxed">
              <span className="font-bold text-white">New gym?</span> Start with Starter — pay only for the members you actually add, no subscription. <span className="font-bold text-white">Growing fast?</span> Growth and Unlimited give you simple, flat monthly pricing so you never have to do the math.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-slate-950 text-slate-400 py-12 border-t border-slate-800 text-sm text-center">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-xs text-blue-400 font-semibold mb-4 tracking-wide uppercase">
            Pay Per Active. No Flat Fees. No Growth Penalty.
          </p>
          <p className="text-xs text-slate-500">© {new Date().getFullYear()} ActiPay Fitness. All rights reserved.</p>
        </div>
      </footer>
    </main>
  );
